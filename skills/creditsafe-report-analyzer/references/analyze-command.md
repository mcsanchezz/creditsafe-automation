...
Turnover with YoY growth rates across available years. Pre-tax profit with YoY changes. Shareholder's funds trajectory (highlight if recovering from negative equity). Balance sheet health — comment on solvency, liquidity and debt indicators where available, and cash flow signals. Outstanding charges — who holds them, what type (fixed/floating), how many outstanding vs satisfied. Workforce trends. Include relevant Creditsafe commentary.
